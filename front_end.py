...
def entry_command():
    for row in back_end.insert(title_text.get(),author_text.get(),year_text.get(),isbn_text.get()):
        list1.delete(0,END)
...
def delete_command():
    global selected_tuple
    back_end.delete(selected_tuple)


def update_command():
    global selected_tuple
    back_end.update(selected_tuple,title_text.get(),author_text.get(),year_text.get(),isbn_text.get())
...
list1.bind('<<ListboxSelected>>',get_selected_row)
#creating buttons
